...
class MtMCrossCurrencySwapDeal(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        self.isQuanto = get_interest_rate_currency(field['Receive_Interest_Rate'], all_factors) != field[
            'Receive_Currency']
        if self.isQuanto:
...
class FXNonDeliverableForward(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        field['Settlement_Currency'] = utils.check_rate_name(self.field['Settlement_Currency'])
        ...
        field_index = {}
        field_index['BuyFX'] = get_fx_and_zero_rate_factor(field['Buy_Currency'], static_offsets,
                                                           stochastic_offsets, all_tenors, all_factors)
        field_index['Discount'] = get_discount_factor(field['Discount_Rate'], static_offsets, stochastic_offsets,
                                                      all_tenors, all_factors)
        field_index['SellFX'] = get_fx_and_zero_rate_factor(field['Sell_Currency'], static_offsets,
                                                            stochastic_offsets, all_tenors, all_factors)
        field_index['SettleFX'] = get_fx_and_zero_rate_factor(field['Settlement_Currency'], static_offsets,
                                                              stochastic_offsets, all_tenors, all_factors)

        field_index['Maturity'] = (self.field['Settlement_Date'] - base_date).days
        # needed for reporting
        field_index['Local_Currency'] = '{0}.{1}'.format(self.field['Buy_Currency'], self.field['Sell_Currency'])
    ...
    def generate(self, shared, time_grid, deal_data):
        ...
        # settle the cash
        pricing.cash_settle(shared, self.field['Settlement_Currency'], deal_data.Time_dep.deal_time_grid[-1], cash[-1])
...
class FXSwapDeal(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        field['NearBuyFarSell_DiscountRate'] = utils.check_rate_name(self.field['Near_Buy_Far_Sell_Discount_Rate'])

        field['NearSellFarBuy_Currency'] = utils.check_rate_name(self.field['Near_Sell_Far_Buy_Ccy'])
        field['NearSellFarBuy_DiscountRate'] = utils.check_rate_name(self.field['Near_Sell_Far_Buy_Discount_Rate'])

        field_index = {}
        field_index['NearBuyFX'] = get_fxrate_factor(field['NearBuyFarSell_Currency'], static_offsets,
                                                     stochastic_offsets)
        field_index['NearBuyDiscount'] = get_discount_factor(field['NearBuyFarSell_DiscountRate'], static_offsets,
                                                             stochastic_offsets, all_tenors, all_factors)
        field_index['NearSellFX'] = get_fxrate_factor(field['NearSellFarBuy_Currency'], static_offsets,
                                                      stochastic_offsets)
        field_index['NearSellDiscount'] = get_discount_factor(field['NearSellFarBuy_DiscountRate'], static_offsets,
                                                              stochastic_offsets, all_tenors, all_factors)
        field_index['Maturity'] = (self.field['Far_Settlement_Date'] - base_date).days
        field_index['Near_Maturity'] = (self.field['Near_Settlement_Date'] - base_date).days
...
class FXForwardDeal(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        field_index['BuyDiscount'] = get_discount_factor(field['Buy_Discount_Rate'], static_offsets, stochastic_offsets,
                                                         all_tenors, all_factors)
        field_index['SellFX'] = get_fxrate_factor(field['Sell_Currency'], static_offsets, stochastic_offsets)
        field_index['SellDiscount'] = get_discount_factor(field['Sell_Discount_Rate'], static_offsets,
                                                          stochastic_offsets,
                                                          all_tenors, all_factors)
        field_index['Maturity'] = (self.field['Settlement_Date'] - base_date).days
...
class SwapInterestDeal(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        field['Discount_Rate'] = utils.check_rate_name(self.field['Discount_Rate']) if self.field['Discount_Rate'] else \
            field['Currency']
        field['Interest_Rate'] = utils.check_rate_name(self.field['Interest_Rate']) if self.field['Interest_Rate'] else \
            field['Discount_Rate']
        ...
        if self.field['Pay_Rate_Type'] == 'Fixed':
            field_index['FixedCashflows'] = utils.generate_fixed_cashflows(base_date, self.paydates,
                                                                           -self.field['Principal'],
                                                                           self.field['Amortisation'],
                                                                           utils.get_day_count(
                                                                               self.field['Pay_Day_Count']),
                                                                           self.field['Swap_Rate'] / 100.0)
            field_index['FloatCashflows'] = utils.generate_float_cashflows(base_date, time_grid, self.recdates,
                                                                           self.field['Principal'],
                                                                           self.field['Amortisation'],
                                                                           self.field['Known_Rates'],
                                                                           self.field['Receive_Interest_Frequency'],
                                                                           self.field['Index_Tenor'],
                                                                           utils.get_day_count(
                                                                               self.field['Receive_Day_Count']),
                                                                           self.field['Floating_Margin'] / 10000.0)
        else:
            field_index['FixedCashflows'] = utils.generate_fixed_cashflows(base_date, self.recdates,
                                                                           self.field['Principal'],
                                                                           self.field['Amortisation'],
                                                                           utils.get_day_count(
                                                                               self.field['Receive_Day_Count']),
                                                                           self.field['Swap_Rate'] / 100.0)
            field_index['FloatCashflows'] = utils.generate_float_cashflows(base_date, time_grid, self.paydates,
                                                                           -self.field['Principal'],
                                                                           self.field['Amortisation'],
                                                                           self.field['Known_Rates'],
                                                                           self.field['Pay_Interest_Frequency'],
                                                                           self.field['Index_Tenor'],
                                                                           utils.get_day_count(
                                                                               self.field['Pay_Day_Count']),
                                                                           self.field['Floating_Margin'] / 10000.0)
        ...
        field_index['Fixed_Compounding'] = utils.CASHFLOW_METHOD_Fixed_Compounding_Yes if self.field[
                                                                                              'Fixed_Compounding'] == 'Yes' else utils.CASHFLOW_METHOD_Fixed_Compounding_No
    ...
    def generate(self, shared, time_grid, deal_data):
        ...
        return pricing.pvfixedleg(shared, time_grid, deal_data) + pricing.pvfloatleg(shared, time_grid, deal_data)
...
class CFFixedInterestListDeal(Deal):
    ...
    def generate(self, shared, time_grid, deal_data):
        return pricing.pvfixedleg(shared, time_grid, deal_data)
...
class CFFixedListDeal(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        field_index['Discount'] = get_discount_factor(field['Discount_Rate'], static_offsets, stochastic_offsets,
                                                      all_tenors, all_factors)
        field_index['Cashflows'] = utils.make_simple_fixed_cashflows(base_date,
...
class FixedCashflowDeal(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        # needed for reporting
        field_index['Local_Currency'] = self.field['Currency']
        return field_index
    ...
    def generate(self, shared, time_grid, deal_data):
        ...
        # settle the cashflow
        pricing.cash_settle(shared, self.field['Currency'], deal_data.Time_dep.deal_time_grid[-1], self.field['Amount'])
...
class CFFloatingInterestListDeal(Deal):
    ...
    def generate(self, shared, time_grid, deal_data):
        return pricing.pvfloatleg(shared, time_grid, deal_data)
...
class YieldInflationCashflowListDeal(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        field_index['ForecastIndex'] = get_inflation_factor(field['Index'], static_offsets, stochastic_offsets,
                                                            all_tenors)
        field_index['PriceIndex'] = get_price_index_factor(field['PriceIndex'], static_offsets, stochastic_offsets)
        field_index['Discount'] = get_discount_factor(field['Discount_Rate'], static_offsets, stochastic_offsets,
                                                      all_tenors, all_factors)
        field_index['Currency'] = get_fxrate_factor(field['Currency'], static_offsets, stochastic_offsets)
    ...
    def generate(self, shared, time_grid, deal_data):
        return pricing.pvindexleg(shared, time_grid, deal_data)
...
class SwaptionDeal(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        field_index['VolSurface'] = get_interest_vol_factor(field['Forecast_Rate_Volatility'], pd.DateOffset(years=2),
                                                            static_offsets, stochastic_offsets, all_tenors)
        ...
        if self.field['Payer_Receiver'] == 'Payer':
            field_index['FixedCashflows'] = utils.generate_fixed_cashflows(base_date, self.paydates, -Principal,
                                                                           Pay_Amortisation,
                                                                           utils.get_day_count(Pay_Day_Count),
                                                                           self.field['Swap_Rate'] / 100.0)
            field_index['FloatCashflows'] = utils.generate_float_cashflows(base_date, time_grid, self.recdates,
                                                                           Principal,
                                                                           Receive_Amortisation, None,
                                                                           self.field['Receive_Frequency'],
                                                                           self.field['Index_Tenor'],
                                                                           utils.get_day_count(Receive_Day_Count),
                                                                           Floating_Margin / 10000.0)
        else:
            field_index['FixedCashflows'] = utils.generate_fixed_cashflows(base_date, self.recdates, Principal,
                                                                           Receive_Amortisation,
                                                                           utils.get_day_count(Receive_Day_Count),
                                                                           self.field['Swap_Rate'] / 100.0)
            field_index['FloatCashflows'] = utils.generate_float_cashflows(base_date, time_grid, self.paydates,
                                                                           -Principal, Pay_Amortisation, None,
                                                                           self.field['Pay_Frequency'],
                                                                           self.field['Index_Tenor'],
                                                                           utils.get_day_count(Pay_Day_Count),
                                                                           Floating_Margin / 10000.0)
    ...
    def post_process(self, accum, shared, time_grid, deal_data, child_dependencies):
        ...
        if factor_dep['Cash_Settled']:
            ...
            F_option, F_swap = tf.split(pvbp, counts)
            spot_option, spot_swap = tf.split(st, counts)
            mn_option, mn_swap = tf.split(mn, counts)
...
class FXDiscreteExplicitAsianOption(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        field = {}
        field['Currency'] = utils.check_rate_name(self.field['Currency'])
        field['Underlying_Currency'] = utils.check_rate_name(self.field['Underlying_Currency'])
        field['Discount_Rate'] = utils.check_rate_name(self.field['Discount_Rate']) if self.field['Discount_Rate'] else \
            field['Currency']
        ...
        field_index['Discount'] = get_discount_factor(field['Discount_Rate'], static_offsets, stochastic_offsets,
                                                      all_tenors, all_factors)
        field_index['Underlying_Currency'] = get_fx_and_zero_rate_factor(field['Underlying_Currency'], static_offsets,
                                                                         stochastic_offsets, all_tenors, all_factors)
        field_index['FX_Volatility'] = get_fx_vol_factor(field['FX_Volatility'], static_offsets,
                                                         stochastic_offsets, all_tenors)
        ...
        field_index['Samples'] = utils.make_sampling_data(base_date, time_grid, self.field['Sampling_Data'])

        # needed for reporting
        field_index['Local_Currency'] = '{0}.{1}'.format(self.field['Underlying_Currency'], self.field['Currency'])
        return field_index
...
class EquityDiscreteExplicitAsianOption(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        field = {}
        field['Currency'] = utils.check_rate_name(self.field['Currency'])
        field['Equity'] = utils.check_rate_name(self.field['Equity'])
        field['Discount_Rate'] = utils.check_rate_name(self.field['Discount_Rate']) if self.field['Discount_Rate'] else \
            field['Currency']
        ...
        field_index['Equity_Zero'] = get_equity_zero_rate_factor(field['Equity'], static_offsets, stochastic_offsets,
                                                                 all_tenors, all_factors)
        field_index['Dividend_Yield'] = get_dividend_rate_factor(field['Equity'], static_offsets, stochastic_offsets,
                                                                 all_tenors)
        field_index['Equity_Volatility'] = get_equity_price_vol_factor(field['Equity_Volatility'], static_offsets,
                                                                       stochastic_offsets, all_tenors)
        field_index['Expiry'] = (self.field['Expiry_Date'] - base_date).days
    ...
    def generate(self, shared, time_grid, deal_data):
        ...
        for index, (discount_block, spot_block, carry_block) in enumerate(
                utils.split_counts([discount, eq_spot, b], counts, shared)):
            t_block = discount_block.time_grid
            sample_t = samples.schedule[start_index[index]:]
            tenor_block = factor_dep['Expiry'] - t_block[:, utils.TIME_GRID_MTM]
            ...
            cash = self.field['Units'] * theo_price
            mtm_list.append(cash * discount_rates)
...
class EquityOptionDeal(Deal):
    ...
    def generate(self, shared, time_grid, deal_data):
        ...
        moneyness = eq_spot / self.field['Strike_Price']
        expiry = daycount_fn(factor_dep['Expiry'] - deal_time[:, utils.TIME_GRID_MTM])
        vols = utils.calc_time_grid_vol_rate(factor_dep['Equity_Volatility'], moneyness, expiry, shared)
        theo_price = utils.black_european_option(forward, self.field['Strike_Price'],
                                                 vols, expiry,
                                                 1.0 if self.field['Buy_Sell'] == 'Buy' else -1.0,
                                                 1.0 if self.field['Option_Type'] == 'Call' else -1.0, shared)
        discount_rates = tf.squeeze(
        ...
        value = self.field['Units'] * theo_price
        mtm = value * discount_rates * fx_rep

        # calculate cashflow settlement (if necessary)
        pricing.cash_settle(shared, self.field['Currency'], deal_data.Time_dep.deal_time_grid[-1], value[-1])
...
class EquityBarrierOption(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        field_index['Dividend_Yield'] = get_dividend_rate_factor(field['Equity'], static_offsets, stochastic_offsets,
                                                                 all_tenors)
        field_index['Volatility'] = get_equity_price_vol_factor(field['Equity_Volatility'], static_offsets,
                                                                stochastic_offsets, all_tenors)
    ...
    def generate(self, shared, time_grid, deal_data):
        return pricing.pveqbarrieroption(shared, time_grid, deal_data)
...
class EquitySwapletListDeal(Deal):
    ...
    def generate(self, shared, time_grid, deal_data):
        return pricing.pvequityleg(shared, time_grid, deal_data)
...
class FXOneTouchOption(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        field_index['Discount'] = get_discount_factor(
            field['Discount_Rate'], static_offsets, stochastic_offsets, all_tenors, all_factors)
        field_index['Underlying_Currency'] = get_fx_and_zero_rate_factor(
            field['Underlying_Currency'], static_offsets, stochastic_offsets, all_tenors, all_factors)
        field_index['Volatility'] = get_fx_vol_factor(
    ...
    def generate(self, shared, time_grid, deal_data):
        return pricing.pvfxonetouch(shared, time_grid, deal_data)
...
class FXBarrierOption(Deal):
    ...
    def generate(self, shared, time_grid, deal_data):
        return pricing.pvfxbarrieroption(shared, time_grid, deal_data)
...
class FXOptionDeal(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        field_index['Underlying_Currency'] = get_fx_and_zero_rate_factor(
            field['Underlying_Currency'], static_offsets, stochastic_offsets, all_tenors, all_factors)
        field_index['FX_Volatility'] = get_fx_vol_factor(field['FX_Volatility'], static_offsets, stochastic_offsets,
                                                         all_tenors)
        field_index['Expiry'] = (self.field['Expiry_Date'] - base_date).days
        field_index['Invert_Moneyness'] = 1 if field['Currency'][0] == field['FX_Volatility'][0] else 0
    ...
    def generate(self, shared, time_grid, deal_data):
        ...
        forward = utils.calc_fx_forward(factor_dep['Underlying_Currency'], factor_dep['Currency'],
                                        factor_dep['Expiry'], deal_time, shared)
        fx_spot = utils.calc_fx_cross(factor_dep['Underlying_Currency'][0], factor_dep['Currency'][0],
                                      deal_time, shared)
        fx_rep = utils.calc_fx_cross(factor_dep['Currency'][0], shared.Report_Currency,
                                     deal_time, shared)
        moneyness = self.field['Strike_Price'] / fx_spot if factor_dep['Invert_Moneyness'] \
            else fx_spot / self.field['Strike_Price']
        expiry = daycount_fn(factor_dep['Expiry'] - deal_time[:, utils.TIME_GRID_MTM])
        fx_vols = utils.calc_time_grid_vol_rate(factor_dep['FX_Volatility'], moneyness, expiry, shared)
        theo_price = utils.black_european_option(forward, self.field['Strike_Price'],
                                                 fx_vols, expiry,
                                                 1.0 if self.field['Buy_Sell'] == 'Buy' else -1.0,
                                                 1.0 if self.field['Option_Type'] == 'Call' else -1.0, shared)
        ...
        value = self.field['Underlying_Amount'] * theo_price
        mtm = value * discount_rates * fx_rep
...
class DealDefaultSwap(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        field_index['Discount'] = get_discount_factor(field['Discount_Rate'], static_offsets, stochastic_offsets,
                                                      all_tenors, all_factors)
        field_index['Name'] = get_survival_factor(field['Name'], static_offsets, stochastic_offsets, all_tenors)
        field_index['Recovery_Rate'] = get_recovery_rate(field['Name'], all_factors)

        pay_rate = self.field['Pay_Rate'] / 100.0 if isinstance(self.field['Pay_Rate'], float) else self.field[
            'Pay_Rate'].amount
    ...
    def generate(self, shared, time_grid, deal_data):
        return pricing.pvcdsleg(shared, time_grid, deal_data)
...
class FRADeal(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        field_index['Forward'] = get_interest_factor(field['Interest_Rate'], static_offsets, stochastic_offsets,
                                                     all_tenors)
        field_index['Daycount'] = utils.get_day_count(self.field['Day_Count'])
        ...
        field_index['VolSurface'] = np.zeros(1, dtype=np.int32)
        field_index['Cashflows'] = utils.make_float_cashflows(base_date, time_grid,
                                                              1 if self.field['Borrower_Lender'] == 'Borrower' else -1,
                                                              cashflows)
...
class FloatingEnergyDeal(Deal):
    ...
    def generate(self, shared, time_grid, deal_data):
        return pricing.pvenergyleg(shared, time_grid, deal_data)
...
class FixedEnergyDeal(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        field_index['Discount'] = get_discount_factor(field['Discount_Rate'], static_offsets, stochastic_offsets,
                                                      all_tenors, all_factors)
        field_index['Cashflows'] = utils.make_energy_fixed_cashflows(base_date, -1.0
...
class EnergySingleOption(Deal):
    ...
    def calc_dependencies(self, base_date, static_offsets, stochastic_offsets, all_factors, all_tenors, time_grid,
        ...
        if field['Currency'] != forward_factor.get_currency():
            ...
            field_index['ImpliedCorrelation'] = get_implied_correlation(('FxRate',) + fx_lookup,
                                                                        ('ReferencePrice',) + forward_price_vol,
                                                                        all_factors)
        ...
        field_index['Discount'] = get_discount_factor(field['Discount_Rate'], static_offsets, stochastic_offsets,
                                                      all_tenors, all_factors)
        field_index['ReferenceVol'] = get_forward_price_vol_factor(forward_price_vol, static_offsets,
                                                                   stochastic_offsets,
                                                                   all_tenors)
        field_index['SettleCurrency'] = self.field['Payoff_Currency']
    ...
    def generate(self, shared, time_grid, deal_data):
        ...
        discount = utils.calc_time_grid_curve_rate(factor_dep['Discount'], deal_time, shared)
        daycount_fn = factor_dep['Discount'][0][utils.FACTOR_INDEX_Daycount]
        buyorsell = 1.0 if self.field['Buy_Sell'] == 'Buy' else -1.0
        callorput = 1.0 if self.field['Option_Type'] == 'Call' else -1.0
        ...
        sample_values = all_samples.gather_weighted_curve(
            ...
            multiply_by_time=False) * tf.expand_dims(fx_spot, axis=1)

        past_samples = tf.squeeze(
            tf.concat([tf.stack(known_samples), sample_values]
                      if known_samples else sample_values, axis=0), axis=1)
        ...
        for index, (forward_block, discount_block) in enumerate(utils.split_counts(
            ...
            average = tf.reduce_sum(
                ...
                samples.schedule[:start_index[index], utils.RESET_INDEX_Weight].reshape(-1, 1),
                axis=0)
            ...
            if sample_t.any():
                ...
                forwardfx = utils.calc_fx_forward(
                    ...
                    sample_t[:, utils.RESET_INDEX_Start_Day], t_block, shared)
                ...
                product_t = sample_ft * tf.exp(
                    np.expand_dims(sample_tenor, axis=2) * vol2)
            ...
            cash = self.field['Volume'] * theo_price
            mtm_list.append(cash * discount_rates)
