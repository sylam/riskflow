"""Validation suite for the Heston-Nandi closed-form GARCH pricer (core in ``riskflow.utils``,
test-only reference helpers in ``tests/hn_reference.py``).
...
from riskflow import utils                                             # noqa: E402
import hn_reference as hnref                                           # noqa: E402
...
def test_no_branch_winding(cfg, n):
    ...
    p = _p(*cfg)
    nodes, _ = utils.gauss_legendre(0.0, 20000.0, 512, 8, DT)
...
def test_expected_sum_h_exact_at_n1_and_matches_mc(h_mult):
    ...
    for n in (5, 21, 63):
        ...
        mc, se = hnref.hn_simulate_sum_h(p, n, h1, 2_000_000, seed=7, device=DEV)
        assert abs(cf - mc) < 4.0 * se, (n, cf, mc, se)
    # per-k path must sum to the closed form
    for n in (5, 21, 63):
        tot = float(hnref.hn_expected_sum_h(p, n, _t(h1)))
        assert abs(float(hnref.hn_expected_h_path(p, n, _t(h1)).sum()) - tot) < 1e-14 * tot
...
def test_true_variance_exceeds_expected_sum_h():
    ...
    for n in (1, 5, 21, 63):
        v = float(hnref.hn_expected_sum_h(p, n, _t(h1)))
        k2 = hnref.hn_cumulants(p, n, _t(h1), 2)[1]
