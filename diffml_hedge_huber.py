...
Objective: protect the DOWNSIDE while keeping the UPSIDE -- a Huber-style downside utility
(linear on gains; quadratic on small losses; linear in the deep tail).  The linear tail
keeps the value bounded and its gradient alive, so the leveraged policy fits cleanly.
...
AVERSION = 2.5        # downside curvature (a) near W=0  -> variance/tail aversion
DELTA    = 5.0        # loss beyond which the penalty goes LINEAR (the Huber knee)
...
def utility(W):
    """Huber-style downside penalty: linear (uncapped) on GAINS so the upside is kept;
    QUADRATIC on small losses (curvature -> tail/variance aversion); LINEAR in the deep
    tail beyond DELTA.  The linear tail bounds the value scale (so the residual stays
    fittable) and keeps a constant, non-vanishing gradient (so deep losses still steer
    the policy) -- unlike a pure quadratic (value explodes) or a hard clamp (gradient dies)."""
...
def sandwich(nets, n_paths, gen):
    # lower bound = the DEPLOYED policy's value.  Use a deployment-quality argmax:
    # n_inner=6 rolls a deliberately noisy version of the policy and badly understates L.
    L = float(greedy_rollout(nets, torch.full((n_paths,), S_INIT),
                             torch.zeros(n_paths), torch.zeros(n_paths), 0, gen, n_inner=24).mean())
...
def main():
    ...
    print(f"    (Huber utility is unbounded above (linear gains), so the clairvoyant upper")
    print(f"     bound is loose; the gap is perfect-foresight slack, not policy suboptimality.)")
    ...
    print("  (Huber downside penalty -> the policy caps the tail AND keeps the upside (95% >>")
    print("   static hedge); AVERSION/DELTA trade tail vs upside, with a clean fit either way.)")
