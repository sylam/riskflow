...
- `calc_time_grid_curve_rate(code, …, n_batch_dims=1)`: builds a `TensorBlock`; `make_curve_tensor` collapses extra trailing batch axes (inner-MC `(B,B2)→B*B2`) when `n_batch_dims>1`, keeping the interp stack single-trailing-batch. The buffer value it reads is a tensor or a `ScenarioSource` (scenario-row blocks, published only by an inner-MC fork); `build_interpolation` turns either into the strategy that reads it — a leaf `Interpolation` for the ordinary case, a `RoutedInterpolation` over per-block leaves for a fork. `n_batch_dims>1` gathers happen during `generate`, before a fork publishes, so they only ever see one block. See [Calc Lifecycle](calc_lifecycle.md#inner-mc-subsystem).
