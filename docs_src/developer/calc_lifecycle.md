...
- **Bundle + runtime**: `build_hedge_bundle` + `construct_hedge_runtime` + `run_hedge_execution`; in `solve_hedge` mode the bundle carries `inner_mc_fn` / `inner_mc_grad_fn` closures that fork inner MC on demand from the cached outer buffer.
