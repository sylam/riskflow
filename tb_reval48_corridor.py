"""REVALIDATION 48-trade corridor RE-ROLL (throwaway; tb_ prefix, untracked, no commit).

Post-redundancy-wave re-validation: sources checkpoints from the reval48_gpu{0,1} training lanes
(market_dim 6 after the CME_FLAT deletion), not the pre-wave garch48 lanes.
...
training happened (checkpoint mtimes unchanged) and that the source rows are spot_model='garch'.
"""
...
os.environ.setdefault('TB_BASE', os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                              'artifacts', 'walk_forward', 'reval48_corridor'))
...
LANES_SRC = [(f'{WF}/reval48_gpu0', '2020-01'), (f'{WF}/reval48_gpu1', '2022-01')]
...
def build_month_map():
    ...
    for src, start in LANES_SRC:
        ...
        for i, m in enumerate(months):
            row = json.load(open(f'{src}/row_{m}.json'))
            assert row.get('spot_model') == 'garch', f'{src}/row_{m}.json is not a garch row'
            md = (f'{src}/md_{m}_fresh{SFX}.json' if row.get('fresh_md')
                  else f'{src}/md_{months[(i // 3) * 3]}{SFX}.json')
...
def main():
    ...
    ap.add_argument('--base', default=os.path.join(WF, 'reval48_corridor'))
    args = ap.parse_args()
