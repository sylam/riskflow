...
Objective: a low-variance protective hedge -- CARA (variance-averse) utility, so the
optimum rejects the profitable-but-volatile drift trade and minimises risk / lifts the tail.
...
AVERSION = 0.20       # CARA risk aversion.  For Gaussian wealth, max E[U] == max (m - a/2 var),
                      # i.e. mean-variance: it penalises ALL variance, so the volatile drift
                      # trade is rejected and the optimum is the low-variance protective hedge.
QGRID  = torch.linspace(-0.25, 2.25, 26)         # admissible spot holdings (shares)
NA     = QGRID.numel()
...
def utility(W):
    """CARA  U(W) = -exp(-a W).  Symmetric, variance-averse, no satiation; the clamp just
    guards exp overflow on rare large-loss paths (it almost never binds once hedged)."""
    return -torch.exp(-AVERSION * torch.clamp(W, min=-25.0))
...
def simulate_bank(n_paths, gen):
    ...
    for t in range(T):
        ...
        q   = (q_rep + 0.6 * torch.randn(n_paths, generator=gen)).clamp(float(QGRID[0]),
                                                                        float(QGRID[-1]))
...
def main():
    ...
    # ---- close the loop: APPLY the measured lambda on a warm-started second sweep ----
    ...
    print(f"    (under bounded CARA the naive bound is tight; the penalised bound is loose")
    print(f"     because the exp-scaled penalty is exploitable by the clairvoyant DP.)")
    ...
    print("  (variance-averse CARA -> the learned policy now MINIMISES risk: low std and a")
    print("   much-lifted tail vs unhedged, rejecting the volatile drift trade.)")
