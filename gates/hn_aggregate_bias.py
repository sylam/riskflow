...
Run:  python gates/hn_aggregate_bias.py            (~3 min on a 3090, ~25 min CPU)
Outputs: gates/hn_aggregate_bias.csv       (main survival-probability error table)
         gates/hn_aggregate_moments.csv    (E[Sum h] validation + exact aggregate moments)
         gates/hn_daily_monitoring.csv     (daily monitoring is a different product)
         gates/hn_last_substep_oss.csv     (the exact drop-in)
         gates/hn_compounding.csv          (per-fixing error compounded over a schedule)
"""
...
if __name__ == '__main__':
    ...
    df3.to_csv('gates/hn_aggregate_bias.csv', index=False)
    df1['section'] = 'expected_sum_h'
    df2['section'] = 'moments'
    pd.concat([df1, df2], ignore_index=True).to_csv('gates/hn_aggregate_moments.csv',
                                                    index=False)
    df4.to_csv('gates/hn_daily_monitoring.csv', index=False)
    df5.to_csv('gates/hn_last_substep_oss.csv', index=False)
    df7.to_csv('gates/hn_compounding.csv', index=False)
    print('\nwrote gates/hn_aggregate_bias.csv, gates/hn_aggregate_moments.csv, '
          'gates/hn_daily_monitoring.csv, gates/hn_last_substep_oss.csv, gates/hn_compounding.csv')
